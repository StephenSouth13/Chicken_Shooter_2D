...
public class Spawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        startPos.z = 0;
        startPos.y -= gridSize; // đẩy xuống 1 ô để khỏi dính mép trên
        startPos.x -= width / 2f; // chỉnh sang trái cho vừa màn hình
    ...
    void SpawnChicken(int rowCount, int colCount)
{
    ...
    for (int i = 0; i < rowCount; i++)
    {
        for (int j = 0; j < colCount; j++)
        {
            ...
            chicken.transform.SetParent(GridChicken, false); // ✅ set parent đúng cách
            pos.x += gridSize; // dịch sang phải
        }
